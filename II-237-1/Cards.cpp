...
struct Cards
{
	vs deal( int numPlayers, string deck )
	{
		vs result(numPlayers);
		for( int i = 0; i + numPlayers <= deck.size(); )
			for( int k = 0; k < numPlayers; ++i, ++k )
				result[k] += deck[i];
		return result;
	}
	// BEGIN CUT HERE
	...
	{"", "" }
	; vs result(_result, _result + (sizeof(_result) / sizeof(_result[0]))); A2.push_back( result );  }
	for( int i = 0; i < A0.size(); i++ ) if ( Case == -1 || i == Case ) { DWORD start, finish; start = timeGetTime(); verify_case( i, A2[i], deal( A0[i], A1[i]) ); finish = timeGetTime(); cout << "Elapsed " << finish - start << " msec" << endl; } }
// END CUT HERE
};
// BEGIN CUT HERE
int main()
