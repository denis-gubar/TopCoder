...
struct GolfScore
{
	...
	{
		vs A;
		A.push_back( "hole in one" ); 
		A.push_back( "triple bogey" );
		A.push_back( "double bogey" );
		A.push_back( "bogey" );
		A.push_back( "par" );
		A.push_back( "birdie" );
		A.push_back( "eagle" );
		A.push_back( "albatross" );
		int result = 0;
		fora(i, parValues)
		{
			int k = distance( A.begin(), find( all(A), scoreSheet[i] ) );
			if ( k )
				result += parValues[i] + 4 - k;
			else
				++result;
		}
		return result;
