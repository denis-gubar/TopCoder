	...
	vector <int> getRange( vector <int> enter, vector <int> exit, int physicalLimit )
	{
		...
		for( int i = 0; i <= physicalLimit; i++ )
		{
			...
			fora(j, enter)
			{
				...
				current -= exit[j];
				if ( current + enter[j] > physicalLimit )
				{
					flag = false; break;
				}
				current += enter[j];
		...
		vi temp(2);
		temp[0] = *min_element( all(result) );
		temp[1] = *max_element( all(result) );
		return temp;
	}
// BEGIN CUT HERE
	public:
	...
	void test_case_3() { int Arr0[] = {0,2}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = {1,0}; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); int Arg2 = 1; vector <int> Arg3; verify_case(3, Arg3, getRange(Arg0, Arg1, Arg2)); }
 ...
 195, 171, 47, 16, 54, 79, 69, 6, 97, 154, 80, 151, 76} ; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); int Arg2 = 954; int Arr3[] = { 453,  659 }; vector <int> Arg3(Arr3, Arr3 + (sizeof(Arr3) / sizeof(Arr3[0]))); verify_case(5, Arg3, getRange(Arg0, Arg1, Arg2)); }
	void test_case_6() { int Arr0[] = {2}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arr1[] = {3}; vector <int> Arg1(Arr1, Arr1 + (sizeof(Arr1) / sizeof(Arr1[0]))); int Arg2 = 2; vector <int> Arg3; verify_case(6, Arg3, getRange(Arg0, Arg1, Arg2)); }
// END CUT HERE
};
// BEGIN CUT HERE 
int main()
